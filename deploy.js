...
const approve = async (spender, token, amount) => {
  console.log('Approving spender = ' + spender + " & token = " + token + " & amount = " + amount + "(" + parseUnits(amount, 18).toString() + ")")
...
const initAMM = async () => {
  ...
  try{
    ...
    const factory = await Factory.deploy(owner.address, {nonce: nonce++});
    ...
    const usdc = await USDC.deploy("USD Coin", "USDC", 1000000, {nonce: nonce++});
    ...
    const weth = await WETH.deploy("Wrapped Eth", "WETH", 1000000, {nonce: nonce++});
    ...
    const router = await Router.deploy(factory.address, weth.address, {nonce: nonce++});
...
const getReserves = async () => {
  const pair = new Contract(
    '0xA7c620407Fd643DAd138b45d766eDDe0C667808C',
    pairArtifact.abi,
    owner
  );
  // const result = await WETH.balanceOf(owner.address);
